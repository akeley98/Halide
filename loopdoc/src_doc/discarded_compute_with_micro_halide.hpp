...
struct LoopNestPrinter
{
    ...
    void build_groups(const std::vector<FuncContents *> &order)
    {
        ...
        for (auto &kv : comp)
        {
            ...
            // Within-group realization order (loopdoc.md section 14 "Member
            // ordering"): the TOPOLOGICAL order of the fuse edges in which each
            // child realizes before its parent. A member A is a child of member B
            // if any stage of A fuses (via compute_with) into any stage of B. The
            // §6 tie-break orders only members the fuse edges leave unordered
            // (e.g. several direct children of one common parent). The group's
            // spine owner -- the member that is nobody's child -- realizes last.
            //
            // Build the per-member child relation (member-level edges).
            std::map<FuncContents *, std::set<FuncContents *>> mem_children; // B -> its child members
            std::map<FuncContents *, int> child_count;                      // A -> # of A's child members
            for (FuncContents *m : grp->members)
            {
                child_count[m] = 0;
            }
            for (FuncContents *child : grp->members)
            {
                for (const StageData &s : child->stages)
                {
                    if (s.has_fuse && s.fuse_parent &&
                        member_set.count(s.fuse_parent.get()))
                    {
                        FuncContents *par = s.fuse_parent.get();
                        if (par != child && mem_children[par].insert(child).second)
                        {
                            child_count[par]++;
                        }
                    }
            ...
            // Kahn-style: emit a member once ALL of its children are emitted
            // (child-before-parent); among ready members break ties by §6.
            std::map<FuncContents *, int> remaining = child_count;
            std::set<FuncContents *> placed;
            while (grp->realize_order.size() < grp->members.size())
            {
                FuncContents *best = nullptr;
                for (FuncContents *m : grp->members) // grp->members is §6-sorted
                {
                    if (placed.count(m) || remaining[m] != 0)
                    {
                        continue;
                    }
                    best = m;
                ...
                placed.insert(best);
                // Recompute remaining for parents: a parent is ready when all its
                // child members are placed.
                for (FuncContents *m : grp->members)
                {
                    if (placed.count(m))
                    {
                    ...
                    int pend = 0;
                    for (FuncContents *c : mem_children[m])
                    {
                        if (!placed.count(c))
                        {
                            pend++;
            ...
            // Spine owner = last realized (the member that is nobody's child).
    ...
    {
        // Body (compute) order (loopdoc.md section 14 "Member ordering"): walk
        // the members in within-group REALIZATION order (child-before-parent)
        // and emit each member's stages as they become *ready*, with two hard
        // constraints -- each Func's own stages in order (s0 before s1 ...), and
        // each parent stage before the children fused onto it.
        //
        // This is a round-robin over members (not a single global lowest-rank
        // pick): we keep emitting a member's stages until it hits a not-yet-ready
        // one, then move to the next member, and loop until all stages are
        // emitted. The "momentum" matters: e.g. in the diagonal case (f child of
        // g, g child of h) once h.s0 is emitted (splicing g.s1 then f.s2) we
        // continue with h.s1, h.s2 BEFORE returning to the now-ready g.s2 -- so
        // g's trailing stage lands after all of h's, not at g's realization slot.
        std::set<StageId> done;
        std::map<FuncContents *, int> next_stage; // next un-emitted stage per member
        for (FuncContents *m : grp->members)
        {
            next_stage[m] = 0;
        }
        // Members in realization order (child-before-parent), the round-robin
        // sweep order.
        const std::vector<FuncContents *> &sweep = grp->realize_order;
        ...
        {
            progress = false;
            for (FuncContents *m : sweep)
            {
                // Advance this member through its stages while each is ready.
                while (next_stage[m] < num_stages(m))
                {
                    int s = next_stage[m];
                    const StageData &sd = m->stages[s];
                    // A fused-child stage is only "ready" once its parent stage
                    // has been emitted (then it is spliced into the parent's
                    // nest by emit_group_body); record it in `result` so the
                    // member's later stages can proceed.
                    if (sd.has_fuse && sd.fuse_parent &&
                        !done.count({sd.fuse_parent.get(), sd.fuse_parent_stage}))
                    {
                        break; // blocked: parent not yet emitted -> move on
                    }
