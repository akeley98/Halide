...
def _resolve_anchor_arg(ctx, spec):
    ...
    if spec == "auto":
        return current
    # Route through ctx.resolve_schedule_arg so the magic [schedule ID] values
    # (terminus/session_output/golden) resolve here too.
    return ctx.resolve_schedule_arg(spec).full_id
...
def cmd_json_compare_cost(args):
    ...
    if rhs_spec is not None:
        rhs = ctx.resolve_schedule_arg(rhs_spec)
    else:
...
def _resolve_schedule_list(ctx, schedule_args):
    ...
    return [ctx.resolve_schedule_arg(s) for s in schedule_args]
